...
function ThemtaiKhoan() {
  ...
  for (var p of u) {
    if (p.email == NewTk.email) {
      alert("Email bị trùng !!");
      return false;
    }

    if (p.username == NewTk.username) {
      alert("User name bị trùng !!");
      return false;
    }
  }
  ...
  setListUser(u);
  addTableKhachHang();
  alert('Thêm tài khoản "' + NewTk.ten + '" thành công.');
  document.getElementById("KhungThemTaiKhoan").style.transform = "scale(0)";
  clearInputFields();
}
function clearInputFields() {
  ...
  document.getElementById("user").value = ""; // Thay "newUser" bằng id của input tên đăng nhập
